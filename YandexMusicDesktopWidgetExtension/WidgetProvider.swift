...
struct WidgetProvider: AppIntentTimelineProvider {
    ...
    func timeline(for configuration: ConfigurationAppIntent, in context: Context) async -> Timeline<MusicEntry> {
        ...
        // policy: .never — НЕ просим систему перезагружаться по таймеру.
        // Иначе саморегулярные перезагрузки (раньше каждые 5с) съедают дневной
        // лимит бюджета WidgetKit, и тогда система ТРОТТЛИТ наши явные
        // reloadAllTimelines() при смене трека → задержка в несколько секунд.
        // Виджет статичный (без живого прогресс-бара), поэтому таймерные
        // обновления не нужны: основное приложение пушит перезагрузку само,
        // мгновенно при каждой смене трека через событийный стрим.
